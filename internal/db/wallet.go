...
func (db *DB) TransferFunds(fromWalletID, toWalletID int64, amount float64) error {
	...
	// Save transactions
	_, err = tx.Exec(ctx, `INSERT INTO transactions (type, from_wallet_id, to_wallet_id, amount, status) VALUES ('transfer', $1, $2, $3, $4)`, fromWalletID, toWalletID, amount, "success")
	if err != nil {
		return fmt.Errorf("failed to log sender transaction: %w", err)
	}

	_, err = tx.Exec(ctx, `INSERT INTO transactions (type, from_wallet_id, to_wallet_id, amount, status) VALUES ('transfer', $1, $2, $3, $4)`, toWalletID, fromWalletID, amount, "success")
	if err != nil {
...
func (db *DB) CreateTransaction(tx *Transaction) error {
	query := `INSERT INTO transactions (type, from_wallet_id, to_wallet_id, amount, fee, description, status) VALUES ($1, $2, $3, $4, $5, $6, $7)`
	err := db.pool.QueryRow(context.Background(), query, tx.Type, tx.FromWalletID, tx.ToWalletID, tx.Amount, tx.Fee, tx.Description, tx.Status)
	if err != nil {
